...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    $password = $cus['Cus_Password']; 

    $stmt = $conn->prepare("UPDATE customer SET Cus_Name=?, Cus_IC=?, Cus_Phone=?, Cus_Email=?, Cus_Password=? WHERE Cus_ID=?");
    $stmt->bind_param("sssssi",$name,$ic,$phone,$email,$password,$id);
    $stmt->execute();
...
input[disabled], input[readonly] { background-color: #e9ecef; }
.password-input {
